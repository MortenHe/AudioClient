...
async function main() {
    ...
    await sftp.rmdir(server_audio_path + "/assets", true);

    //gezippte JSON-files hochladen
    console.log("upload json zip file");
    await sftp.fastPut("../../myJson.zip", server_audio_path + "/myJson.zip");
    ...
    await ssh.exec("cd " + server_audio_path + " && unzip myJson.zip");

    //Zip-File loeschen
    console.log("delete zip file");
    await sftp.delete(server_audio_path + "/myJson.zip");
